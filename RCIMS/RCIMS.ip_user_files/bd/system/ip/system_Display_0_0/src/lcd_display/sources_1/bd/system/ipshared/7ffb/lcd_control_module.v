...
(
	...
	initial
    rstn=1;
	parameter [23:0] desktop = 24'h6b69d6;
	parameter [23:0]bar_data = 24'h30428e;  // 单色显示的颜色
//-------------------------//
//-----第一个字符的显示----//
//-------------------------//
...
//------灰色对话框显示-----//
//-------------------------//
wire red_ready;  //判定是否进入显示区域
wire white_ready;
parameter [23:0] red_RGB = 24'hc62f2f;
parameter [23:0] white_RGB = 24'hffffff;
assign red_ready = (ready_sig && column_addr_sig >= 120 && column_addr_sig <= 360 && row_addr_sig >= 68 && row_addr_sig <= 84)? 1:0;
assign white_ready = (ready_sig && column_addr_sig >= 120 && column_addr_sig <= 360 && row_addr_sig >= 68 && row_addr_sig <= 204)? 1:0;
//-------------------------//
//------小数点显示-----//
//-------------------------//
wire dot_ready;
assign dot_ready = (ready_sig && column_addr_sig <= 256 && column_addr_sig >= 255 && row_addr_sig >= 143&& row_addr_sig <= 144)? 1:0;
//-------------------------//
//-----输出RGB颜色数据-----//
//-------------------------//
	...
	if(dot_ready)
     begin
     red_sig   = bar_data[23:16];
     green_sig = bar_data[15:8];
     blue_sig  = bar_data[7:0];
     end
 else
	if(char_ready_0)
		begin
		red_sig   = rom_data_0[`WIDTH'd`WIDTH - char_m_0]? bar_data[23:16]:white_RGB[23:16];
		green_sig = rom_data_0[`WIDTH'd`WIDTH - char_m_0]? bar_data[15:8]:white_RGB[15:8];
		blue_sig  = rom_data_0[`WIDTH'd`WIDTH - char_m_0]? bar_data[7:0]:white_RGB[7:0];
		end
	else
	if(char_ready_1)
		begin
		red_sig   = rom_data_1[`WIDTH'd`WIDTH - char_m_1]? bar_data[23:16]:white_RGB[23:16];
		green_sig = rom_data_1[`WIDTH'd`WIDTH - char_m_1]? bar_data[15:8]:white_RGB[15:8];
		blue_sig  = rom_data_1[`WIDTH'd`WIDTH - char_m_1]? bar_data[7:0]:white_RGB[7:0];
end
	else
	if(char_ready_2)
		begin
		red_sig   = rom_data_2[`WIDTH'd`WIDTH - char_m_2]? bar_data[23:16]:white_RGB[23:16];
		green_sig = rom_data_2[`WIDTH'd`WIDTH - char_m_2]? bar_data[15:8]:white_RGB[15:8];
		blue_sig  = rom_data_2[`WIDTH'd`WIDTH - char_m_2]? bar_data[7:0]:white_RGB[7:0];
		end
	else
	if(char_ready_3)
        begin
        red_sig   = rom_data_3[`WIDTH'd`WIDTH - char_m_3]? bar_data[23:16]:white_RGB[23:16];
        green_sig = rom_data_3[`WIDTH'd`WIDTH - char_m_3]? bar_data[15:8]:white_RGB[15:8];
        blue_sig  = rom_data_3[`WIDTH'd`WIDTH - char_m_3]? bar_data[7:0]:white_RGB[7:0];
        end
    else
	if(char_ready_4)
        begin
        red_sig   = rom_data_4[`WIDTH'd`WIDTH - char_m_4]? bar_data[23:16]:white_RGB[23:16];
        green_sig = rom_data_4[`WIDTH'd`WIDTH - char_m_4]? bar_data[15:8]:white_RGB[15:8];
        blue_sig  = rom_data_4[`WIDTH'd`WIDTH - char_m_4]? bar_data[7:0]:white_RGB[7:0];
        end
    else
	if(char_ready_5)
        begin
        red_sig   = rom_data_5[`WIDTH'd`WIDTH - char_m_5]? bar_data[23:16]:white_RGB[23:16];
        green_sig = rom_data_5[`WIDTH'd`WIDTH - char_m_5]? bar_data[15:8]:white_RGB[15:8];
        blue_sig  = rom_data_5[`WIDTH'd`WIDTH - char_m_5]? bar_data[7:0]:white_RGB[7:0];
        end
    else
	if(char_ready_6)
        begin
        red_sig   = rom_data_6[32'd32 - char_m_6]? bar_data[23:16]:white_RGB[23:16];
        green_sig = rom_data_6[32'd32 - char_m_6]? bar_data[15:8]:white_RGB[15:8];
        blue_sig  = rom_data_6[32'd32 - char_m_6]? bar_data[7:0]:white_RGB[7:0];
        end
    else
    if(red_ready)
    begin
        red_sig   = red_RGB[23:16];
        green_sig = red_RGB[15:8];
        blue_sig  = red_RGB[7:0];
    end
    else
    if(white_ready)
    begin
        red_sig   = white_RGB[23:16];
        green_sig = white_RGB[15:8];
        blue_sig  = white_RGB[7:0];
    end
    else
		begin
		red_sig   = desktop[23:16];
	    green_sig = desktop[15:8];
		blue_sig  = desktop[7:0];
		end
